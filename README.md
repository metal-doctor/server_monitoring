...
Для безопасного подключения к grafana и передачи показателей graphite-exporter используется реверс прокси nginx. Он использует самоподписанный сертификат.
